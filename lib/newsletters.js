...
export function requireAdmin(request) {
  ...
  if (!tenant) throw Response.json({ error: "Niet ingelogd" }, { status: 401 });
  if (role !== "admin") throw Response.json({ error: "Alleen admins" }, { status: 403 });
  return { tenant, role, userId, userName };
}
...
export async function buildRecipientsForCampaign(tenant, segmentId, excludedSegmentIds = []) {
  const { includeSegment, excludedSegments } = await resolveCampaignSegments(tenant, segmentId, excludedSegmentIds);

  const { data: leads, error: leadError } = await supabaseAdmin
    .from("leads")
    .select("id, tenant, company_name, contact_person, contact_first_name, contact_last_name, email, status, relationship_type, hubspot_deal_origin, industry, marketing_consent, marketing_segments, marketing_subscription_status, marketing_hard_bounced")
    .eq("tenant", tenant)
...
export async function sendTestEmail({ tenant, campaign, toEmail, userName }) {
  ...
  try {
    ...
    const result = await resend.emails.send({
      ...
      html: ensureUnsubscribeLink(campaign.body_html).replaceAll("{{{RESEND_UNSUBSCRIBE_URL}}}", "https://resend.com"),
...
export async function syncAndSendCampaign({ tenant, campaign, userName }) {
  ...
  try {
    ...
    const createdBroadcast = await resend.broadcasts.create({
      ...
      html: ensureUnsubscribeLink(campaign.body_html),
    });
    ...
    await recordNewsletterEvent({
      ...
      resend_broadcast_id: broadcastId,
      payload: { recipient_count: snapshot.length, sent_by: userName, scheduled_at: scheduledAt, resend_response: sent.data || null },
    });

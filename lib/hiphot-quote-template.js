...
export function generateQuoteHtml({
  ...
  const itemRows = (lineItems || [])
    ...
    .map(
      ...
      <tr>
        ...
        <td style="padding:14px 12px;border-bottom:1px solid #f0f0f0;text-align:right;font-size:14px;vertical-align:top;white-space:nowrap;">${formatEuro(item.unit_price)}</td>
        <td style="padding:14px 12px;border-bottom:1px solid #f0f0f0;text-align:right;font-size:14px;font-weight:600;vertical-align:top;white-space:nowrap;">${formatEuro(item.line_total)}${Number(item.discount_pct) > 0 ? `<div style="font-size:11px;color:#999;font-weight:400;white-space:nowrap;">na ${item.discount_pct}% korting</div>` : ""}</td>
      </tr>`
    )
    .join("");
  ...
  const shippingRow =
    totals.verzendkostenOntvangen > 0
      ? `<tr><td colspan="3" style="padding:10px 12px;text-align:right;font-size:14px;color:#555;">${tr.shipping}</td><td style="padding:10px 12px;text-align:right;font-size:14px;white-space:nowrap;">${formatEuro(totals.verzendkostenOntvangen)}</td></tr>`
      : `<tr><td colspan="3" style="padding:10px 12px;text-align:right;font-size:14px;color:#555;">${tr.shipping}</td><td style="padding:10px 12px;text-align:right;font-size:14px;color:#777;white-space:nowrap;">€ 0,00<div style="font-size:11px;white-space:nowrap;">na 100% korting</div></td></tr>`;
  ...
  const introSection = intro
    ? `<div style="margin:0 0 32px;padding:24px 28px;background:#fff;border:1px solid #e5e5e5;border-radius:6px;">
        ...
        <div class="hh-rich" style="font-size:14px;line-height:1.7;color:#444;">${intro}</div>
      </div>`
    : "";
  ...
  const termsSection = termsHtml
    ? `<div style="margin:36px 0 24px;">
        ...
        <div class="hh-rich" style="font-size:13px;line-height:1.7;color:#555;">${termsHtml}</div>
      </div>`
    : "";

  // Remarks kunnen platte tekst zijn (nieuw, vanaf AI-rationale) of HTML (legacy of
  // handmatig gewrapt). Detecteer en wrap platte tekst zelf zodat de klant nette
  // paragrafen ziet i.p.v. één lange regel.
  const formatRemarks = (raw) => {
    const s = String(raw || "").trim();
    if (!s) return "";
    if (/<\/?(p|div|br|strong|em|a|ul|ol|li|h[1-6])\b/i.test(s)) return s;
    return s
      .split(/\n\n+/)
  ...
  const remarksSection = quote.remarks_html
    ? `<div style="margin:24px 0;padding:18px 22px;background:#fffbf0;border-radius:6px;border-left:3px solid #ffc107;">
        <div class="hh-rich" style="font-size:14px;line-height:1.6;color:#555;">${formatRemarks(quote.remarks_html)}</div>
      </div>`
    : "";

  const contactName = quote.contact_name || "HIPHOT team";
  const contactTitle = quote.contact_title || "";
  const contactEmail = quote.contact_email || "hallo@hiphot.nl";
  const contactPhone = quote.contact_phone || "+31 (0)85 505 56 64";
...
<style>
  ...
  * { margin: 0; padding: 0; box-sizing: border-box; }
  /* Rich-text content (branchetekst, opmerkingen, voorwaarden) — herstelt margins die door * { margin:0 } worden gewist */
  .hh-rich p { margin: 0 0 12px; }
  .hh-rich p:last-child { margin-bottom: 0; }
  .hh-rich h2 { font-size: 16px; font-weight: 700; margin: 16px 0 8px; color: #1a1a1a; }
  .hh-rich h3 { font-size: 15px; font-weight: 700; margin: 14px 0 6px; color: #1a1a1a; }
  .hh-rich ul, .hh-rich ol { margin: 0 0 12px 20px; padding-left: 4px; }
  .hh-rich li { margin: 4px 0; }
  .hh-rich blockquote { margin: 0 0 12px; padding: 8px 14px; border-left: 3px solid #FFD500; color: #555; }
  .hh-rich a { color: #1a1a1a; text-decoration: underline; }
  .hh-rich strong { font-weight: 700; }
  .hh-rich em { font-style: italic; }
  ...
  table.hh-items thead th:nth-child(2) { text-align: center; width: 100px; }
  table.hh-items thead th:nth-child(3) { text-align: right; width: 130px; }
  table.hh-items thead th:nth-child(4) { text-align: right; width: 140px; }
...
<div class="hh-page">
  ...
  <div class="hh-body">
    ...
    <table class="hh-items">
      ...
      <tfoot>
        ${totals.artikelKortingen > 0 ? `
        <tr>
          <td colspan="3" style="padding:10px 12px;text-align:right;font-size:14px;color:#555;">${tr.subtotal}</td>
          <td style="padding:10px 12px;text-align:right;font-size:14px;white-space:nowrap;">${formatEuro(totals.brutoVerkoop)}</td>
        </tr>
        <tr>
          <td colspan="3" style="padding:10px 12px;text-align:right;font-size:14px;color:#c84a00;">Korting</td>
          <td style="padding:10px 12px;text-align:right;font-size:14px;color:#c84a00;white-space:nowrap;">- ${formatEuro(totals.artikelKortingen)}</td>
        </tr>
        <tr>
          <td colspan="3" style="padding:10px 12px;text-align:right;font-size:14px;color:#555;">Netto prijs</td>
          <td style="padding:10px 12px;text-align:right;font-size:14px;font-weight:600;white-space:nowrap;">${formatEuro(totals.nettoNaArtikelKorting)}</td>
        </tr>
        ` : `
        <tr>
          <td colspan="3" style="padding:12px;text-align:right;font-size:14px;color:#555;">${tr.subtotal}</td>
          <td style="padding:12px;text-align:right;font-size:14px;font-weight:600;white-space:nowrap;">${formatEuro(totals.nettoNaArtikelKorting)}</td>
        </tr>
        `}
        ${shippingRow}
        <tr class="hh-total-row">
          <td colspan="3" style="padding:14px 12px;text-align:right;border-top:2px solid #1a1a1a;">${tr.total}</td>
          <td style="padding:14px 12px;text-align:right;border-top:2px solid #1a1a1a;white-space:nowrap;">${formatEuro(totals.nettoVerkoop)}</td>
        </tr>
      </tfoot>
    </table>
  ...
  <div class="hh-contact">
    ...
    <div>
      ...
      ${contactTitle ? `${escHtml(contactTitle)}<br>` : ""}
      ${escHtml(contactEmail)}<br>
      ${escHtml(contactPhone)}
    </div>

 ...
 */
export function calculateOrderTotals(items, settings = {}, useFulfillment = true) {
  ...
  // Shipping: we always pay shipping internally, customer pays only under threshold
  const verzendkostenInkoop = Number(s.verzendkosten);
  const verzendkostenOntvangen =
    brutoVerkoop >= Number(s.gratis_drempel) ? 0 : Number(s.verzendkosten);

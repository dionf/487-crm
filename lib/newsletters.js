...
const HEALTH_EVENT_TYPES = {
  ...
  unsubscribed: new Set(["contact.updated", "suppression.added"]),
};
...
async function sendCampaignBatch({ tenant, campaign, batchNumber, userName }) {
  ...
  const hasMore = batchNumber < Number(campaign.batch_total_count || 0);
  const nextRunAt = hasMore ? batchWaitUntil(campaign.batch_wait_hours) : null;
  ...
  const { data: updated, error: updateError } = await supabaseAdmin
    .from("newsletter_campaigns")
    .update({
      status: hasMore ? "batch_waiting" : "sent",
...
export async function startAutomaticBatchCampaign({ tenant, campaign, userName }) {
  ...
  return sendCampaignBatch({ tenant, campaign: prepared, batchNumber: 1, userName });
}
...
export async function calculateBatchHealth(tenant, campaign, batch) {
  ...
  for (const event of events || []) {
    ...
    for (const [key, eventTypes] of Object.entries(HEALTH_EVENT_TYPES)) {
      if (eventTypes.has(event.event_type)) seen[key].add(email);
    }
  }
  ...
  if (tenant === "hiphot") {
    ...
    try {
      if (process.env.HIPHOT_HUB_WHATSAPP_URL) {
        ...
        url.searchParams.set("phone", process.env.CALLMEBOT_PHONE || "+31641917711");
...
export async function processAutomaticBatchCampaign({ tenant, campaign, userName = "CRM cron" }) {
  ...
  return sendCampaignBatch({ tenant, campaign: refreshedCampaign, batchNumber: nextBatchNumber, userName });
}

export async function syncAndSendCampaign({ tenant, campaign, userName }) {
  if (campaign.batch_mode === "automatic") {
    return startAutomaticBatchCampaign({ tenant, campaign, userName });
  }

...
export async function registerFailedAttempt({
  ...
  // `limit` betekent hier iets anders dan in enforceRateLimit. Daar is het het
  // aantal toegestane requests, hier het aantal mislukte pogingen dat tot een
  // lockout leidt. Afgaan op result.allowed (hits <= limit) zou de vijfde
  // misser nog vrijuit laten gaan: er stond dan geen blokkade, dus een juiste
  // pincode of een geraden cron-secret op poging zes kwam er alsnog door.
  if (result.hits < config.limit) return false;

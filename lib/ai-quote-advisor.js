...
// Maximaal aantal tekens van een chatbotgesprek dat in de prompt gaat.
...
- \`shipping_cost\`: laat op 0 — de server bepaalt zelf (NL/BE €3,99, overige EU €4,95, gratis vanaf €199 subtotal). Wat je hier invult wordt genegeerd.
 ...
 *   - submissions: [{ source, conversation_data, conversation_transcript, message, contact }]
...
export function buildLeadContextText(leadContext) {
  ...
  if (leadContext.lead) {
    ...
    if (l.language) leadLines.push(`- Offertetaal: ${l.language}`);
  ...
  if (Array.isArray(leadContext.submissions) && leadContext.submissions.length > 0) {
    ...
    for (const s of leadContext.submissions) {
      ...
      if (s.conversation_transcript) {
        // Een adviesgesprek loopt makkelijk in de duizenden tekens; afkappen zodat
        // één inzending de prompt niet opblaast. Het begin bevat de situatie, dus
        // de staart is het minst waardevol.
        const transcript = String(s.conversation_transcript);
        const trimmed = transcript.length > TRANSCRIPT_LIMIT
          ? `${transcript.slice(0, TRANSCRIPT_LIMIT)}\n[…afgekapt, volledige gesprek staat in de inbox]`
          : transcript;
        lines.push(`  Volledig chatbotgesprek:\n  ${trimmed.split("\n").join("\n  ")}`);
      } else if (!s.conversation_data && s.message) {
        // Alleen terugvallen op de samenvatting als er geen rijkere bron is.
...
Stel een passend offerte-voorstel samen volgens de regels. Schrijf de klant-zichtbare rationale in de offertetaal van de lead. Als essentiële info ontbreekt (aantal personen, buiten/binnen, plaatsing), stel dan in je message een of twee concrete vragen aan de medewerker voordat je een definitief voorstel doet.`,
    });
  } else {
